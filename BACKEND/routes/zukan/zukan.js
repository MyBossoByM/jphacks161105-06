...
router.post('/:username', function(request, response){
	...
	// 追加登録
		...
		// console.log("-------------before obj.characterID start-------------------");
		// console.log(obj.characterID);
		// console.log("-------------before obj.characterID end---------------------");
		for (var i=0; i < idarray.length; i++){
		    obj.characterID.push(idarray[i]);
		}
		// 確認用
		// console.log("-------------after obj.characterID start-----------------");
		// console.log(obj.characterID);
		// console.log("-------------after obj.characterID end-------------------");
		Zukan.update({ "username" : username }, 
				 ...
				 Zukan.find({ "username" : username }, function(err, result){
				     if (err)
					 console.log(err);
				     // console.log("-------------updated data start---------------");
				     // console.log(result);
